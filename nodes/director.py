...
class MiniMaxH3DirectorPlus:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "postprocess_mode": (["native", "lanczos", "ai_upscale", "video_sr", "rtx_vsr"], {"default": "ai_upscale", "tooltip": "H3 音色参考的智能 1080p 使用保守 AI 超分，避免 SeedVR2 二次扩散改写人物细节；SeedVR2 保留给兼容的高分辨率视觉路线"}),
            ...
            "optional": {
                ...
                "face_refine_mode": (["off", "auto"], {"default": "off", "tooltip": "关闭时完全旁路；开启后修复所选 Picture 的单个人物，需原始身份图和身份/切镜检测依赖。请先检查跟踪，不保证画质提升。"}),
                "first_image": ("IMAGE",),
                "last_image": ("IMAGE",),
    ...
    def build(
        ...
        if two_stage_plan is not None and postprocess_path == "balanced_fhd_downscale":
            ...
            if not two_stage_plan.get("qhd_direct"):
                request["warnings"].append(
                    "1080p 二采结果已接近目标尺寸，默认不再执行 SeedVR2 3B 重建，以保留原生清晰度。"
                )
