...
- 校验输出形状为 `720×1280×3`。
- 无论成功或失败，都显式关闭 VSR 效果并解除测试输入/输出引用；不得调用全局 `torch.cuda.empty_cache()`，以便后续 H3 或多 GPU 任务复用分配器缓存。
- 成功日志记录质量、GPU 和探测尺寸。
...
- 单帧成功后若关闭效果失败，也必须在 H3 采样前失败；已有主错误时，关闭失败只记录警告并保留主错误链。
- 成功和失败均关闭效果并清理临时张量。
