...
H3 原生音色参考：高显存且二采依赖满足时，智能1080p使用训练型视频二采，音频零噪声、零重绘遮罩并精确回填首采音频；不再强制退回原生20步。预算/依赖不足仍走原生20步加AI超分，Fish单采与8G既有策略不变。1080p二采后直接导出；声纹、口型、画质尚需GPU对照。详见 [REF2VA音色二采](REF2VA音色二采.md)。 API无新增必填字段。智能预设自动规划；显式 `quality_two_stage` 仍遵守后处理兼容枚举。查看guide中的实际首采/二采尺寸、`postprocess_path` 和 `two_stage_audio_guard`，不要把最终输出尺寸当作原生采样尺寸。
