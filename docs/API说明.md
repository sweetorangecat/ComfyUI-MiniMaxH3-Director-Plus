...
API 模板默认使用 `performance_preset=免费智能 1080p`、`postprocess_mode=ai_upscale`、`ai_upscale_model=auto`、`motion_smoothing=off`。两个 UNETLoader（FL2VA/REF2VA）默认都指向 U22 验证的 INT8 混合底模 `minimax/minimax_h3_hybrid_fl2va_ref2va_b25-49-int8.safetensors`——pruned convrot/nvfp4 重量化底模与 SageAttention 的 INT8/FP8 核叠加会产生全画面编织纹与音频杂质，训练型二采路线必须使用该混合底模（详见《故障排查》“二采画面有编织状纹路”）。这是本地免费路线，不调用付费服务；普通显存解析为 20 步 `quality_sage`（SageAttention），低显存才切换受限路线。生成阶段分辨率不等于最终输出分辨率；H3 原生采样完成后才执行一次细节重建，最终目标按 `resolution_preset` 计算。带 H3 音色参考的智能 1080p 固定使用原生 20 步单采加保守 AI 超分，避免 SeedVR2 的第二次扩散重绘脸部、口型、毛发与服装。无音色的 2K/4K 视觉路线仍可使用 SeedVR2；运行时若其节点或权重未就绪，会在明确警告后回退为 `postprocess_path=ai_upscale`。8GB 低显存训练型二采按目标分辨率限制时长：1080p 最多 6 秒，768p 最多 10 秒，并在启动前检查空闲显存；该路线会把默认 `auto` 按实际倍率解析为 X2 模型（命中 `RealESRGAN_x2plus.pth`）。`resolution_preset` 选择 `2K QHD` / `4K UHD` 时，`免费智能 1080p` 自动解析为 `quality_two_stage` + SeedVR2 链：训练型 latent 二采之后由 SeedVR2 扩散超分到最终 2K/4K 尺寸；要求训练型二采依赖与 SeedVR2 均就绪且显存满足档位（20–24GB 显卡 2K 限 8 秒以内，4K 需 28GB 以上显存），Fish S2 声纹锁定与 2K/4K 互斥（最高 1080p），条件不满足会在排队前报错并给出具体原因。
...
REF2VA 的训练型 reference latent 二采（`trained_latent_ref`）默认保留给无音色视觉任务；8–12GB、`768p H3`、4–10 秒且启用 H3 音色参考时是受保护的例外，音频分支会被锁定并在二采后精确回填。其他带 H3 音色参考的请求统一使用 `ref_quality_native` 原生 20 步单采。旧工作流里保存的带音色 `quality_two_stage` 状态会自动迁移并在 warning 中说明原因。

`postprocess_mode` 控制最终视频输出。智能 1080p 的默认值是 `ai_upscale`：H3 原生音色参考任务以保守的一次 AI 超分保留已生成的角色、口型和毛发。8–12GB、`768p H3`、4–10 秒的 REF2VA 是受保护的例外：依赖齐全时使用 `trained_latent_ref` 二采，音频分支采用零噪声、零重绘遮罩和首采 audio latent 精确回填。无音色的 2K/4K 视觉质量路线推荐 `video_sr`（SeedVR2 扩散视频超分，整段一次处理、时间一致性较好）；`native`（原生尺寸直出）、`lanczos`（CPU Lanczos 快速放大）和 `rtx_vsr`（RTX VSR AI 细节重建）保留给兼容载荷。选择 `video_sr` 时 guide 会附带 `video_sr_plan`（DiT 权重、block swap、VAE 分块与批帧数，按显存自动分档并优先 7B sharp 变体），且 `upscale_method=seedvr2`；SeedVR2 节点或 `models/SEEDVR2/` 权重缺失时不再报错，而是自动回退为 `postprocess_path=ai_upscale` 并在警告中列出缺失项与安装方式。无新增必填入参：无音色质量二采可传 `performance_preset=quality_two_stage`、`postprocess_mode=video_sr`；带 H3 音色参考的同类旧载荷会改走 `ref_quality_native + ai_upscale`。旧的 `postprocess_mode=rtx_vsr` 载荷仍会执行 RTX VSR 并固定 `HIGHBITRATE_ULTRA`，不再启用会产生彩条/花屏风险的 `DEBLUR_LOW` 双效果链。精确 `1080p FHD` 是内部特例，无新增 API 入参：28GB+ 且至少 24GB 空闲显存时规划 `1280×704 -> 1920×1056 -> 1920×1080`；20–24GB 级显卡或空闲显存不足 24GB 的更高档显卡，在至少保留 18GB 空闲显存时规划 `1280×704 -> 1920×1056 -> 1920×1080`，返回 `vram_safety_tier=16_24gb_fhd`。两种 FHD 路线都返回 `postprocess_path=balanced_fhd_downscale`、`upscale_method=aspect_lanczos_downscale`，中心等比裁切并 Lanczos 对齐，不探测或执行 RTX VSR。`低显存二采` 只允许 `ai_upscale`，并按二采后的实际倍率自动优先选择 X2 模型，当前推荐和默认命中 `RealESRGAN_x2plus.pth`。除 FHD 最终尺寸对齐特例外，两种二采都不会叠加另一种放大器。`ai_upscale_model` 为 `auto` 或 `models/upscale_models` 中已安装的模型名；自动解析使用神经二采尺寸而不是首采尺寸，避免错误选择 X4。`rtx_quality` 的公共枚举为 `HIGH`、`ULTRA`、`HIGHBITRATE_ULTRA`，仅在旧 `rtx_vsr` 路线生效。最终只保存一个视频；`video_sr` 以外的路线，实际需要的 AI 模型或 RTX VSR 依赖缺失时会在生成前明确报错，不会静默回退。
...
API 中的 `resolution_preset` 表示请求的最终输出目标，支持精确 `768p H3`（1344×768）、`1080p FHD`（1920×1080）、`2K QHD`（2560×1440）和 `4K UHD`（3840×2160）。4K 不是 H3 原生采样。8GB 低显存选择 `768p H3` 支持 4–15 秒；无音色 FL/T2VA 且低显存二采依赖齐全时，4–10 秒使用训练型 latent 放大和低 sigma 重绘，7–10 秒强制时空分块，并把最终逐帧重建控制在约 1.75 倍以内，避免把约 704×384 的单采结果直接放大到 1344×768。11–15 秒仍保留降低原生网格的兼容单采路线。选择 `1080p FHD` 或更高目标时仍按对应显存预算限制时长，8GB 的 1080p 训练型二采最多 6 秒。20–24GB 级显卡在至少有 18GB 空闲显存时可执行 4–15 秒精确 FHD 保守二采，但 2K 仍只开放 8 秒以内；28GB 以上才允许通过前置检查后请求长视频 2K/4K。所有路线仍由同一个流式 MP4 输出节点保存。
