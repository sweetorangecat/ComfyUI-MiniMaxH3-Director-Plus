...
def _replace_video_vae_decoders(workflow):
    ...
    for container in containers:
        for node in container.get("nodes", []) or []:
            ...
            node["title"] = node.get("title") or "H3 自适应视频 VAE 解码（GPU/CPU FP16）"
            replaced += 1
    return replaced
...
def build_api_template():
    return {
        ...
        **{
        ...
        "22": {"class_type": "MiniMaxH3SafeVAEDecode", "inputs": {"samples": ["21", 0], "vae": ["4", 0]}, "_meta": {"title": "API 自适应视频 VAE 解码（GPU/CPU FP16）"}},
